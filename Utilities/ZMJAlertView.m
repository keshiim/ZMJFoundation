...
#import "ZMJAlertView.h"

@interface ZMJAlertView () <UIAlertViewDelegate>

@end

@implementation ZMJAlertView
{
	NSMutableDictionary *_actionsPerIndex;
    
	ZMJAlertViewBlock _cancelBlock;
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
	...
	ZMJAlertViewBlock block = [_actionsPerIndex objectForKey:key];
	if (block) {
		block();
	}
...
- (void)setDelegate:(id <UIAlertViewDelegate>)delegate
{
    ...
    [NSException raise:@"ZMJFoundationException" format:@"Don't set delegate, set externalDelegate instead."];
}
