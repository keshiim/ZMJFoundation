...
#import "ZMJObjectCache.h"
#import <UIKit/UIKit.h>
#import "NSString+ZMJAdditions.h"

static NSString *const ZMJDataCacheDirectoryName = @"com.cocoawind.ZMJObjectCache";

static const NSInteger kDefaultCacheMaxCacheAge = 60 * 60 * 24 * 7;     // 1 week
static const NSUInteger kDefaultCacheMaxCacheSize = 1024 * 1024 * 512;   // 512MB

@interface ZMJObjectCache ()
...
@implementation ZMJObjectCache

+ (instancetype)sharedCache
{
    static ZMJObjectCache *_cache = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _cache = [[ZMJObjectCache alloc] init];
    });
    return _cache;
}
...
- (id)init
{
    ...
    if (self) {
        ...
        _memCache.name = ZMJDataCacheDirectoryName;
        
        _maxCacheAge = kDefaultCacheMaxCacheAge;
        _maxCacheSize = kDefaultCacheMaxCacheSize;
        
        // Create IO serial queue
        _ioQueue = dispatch_queue_create("com.cocoawind.ZMJObjectCache", DISPATCH_QUEUE_SERIAL);
...
- (NSString *)cachePath
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cachePath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:ZMJDataCacheDirectoryName];
    });
    return cachePath;
}
...
{
    NSString *md5FileName = [key ZMJ_md5];
